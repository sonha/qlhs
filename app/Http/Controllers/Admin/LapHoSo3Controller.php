...
class LapHoSo3Controller extends Controller
{
	...
	public function hoTroAnTruaTreEm(Request $req){
		$json = [];
		$files =  $req->file('file');
        ...
        $checkReportName = DB::table('qlhs_hosobaocao')->where('report_name', 'LIKE', '%'.$report_name.'%')->where('report', 'LIKE', '%HTAT%')->get();
		...
		->select('kp1.money','profile_id','profile_name','profile_year', 'level_old','level_cur','level_new',
			...
			DB::raw('MAX(CASE when profile_subject_subject_id = 26 then 1 else 0 END) DT1'),
			DB::raw('MAX(CASE when profile_subject_subject_id = 27 then 1 else 0 END) DT2'),
			DB::raw('MAX(CASE when profile_subject_subject_id = 28 then 1 else 0 END) DT3'),
			DB::raw('MAX(CASE when profile_subject_subject_id = 19 then 1 else 0 END) DT4'),
			DB::raw("MAX(CASE when level_old <> '' and qlhs_profile.profile_year < '".$namhoc."-06-01' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".$namhoc."-01-01')) then 1 else 0 END) 'HKII1'"),
			DB::raw("MAX(CASE when level_cur <> '' and qlhs_profile.profile_year < '".$namhoc."-12-31' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".$namhoc."-05-31')) then 1 else 0 END) 'HKI2'"),
			DB::raw("MAX(CASE when level_cur <> '' and qlhs_profile.profile_year < '".($namhoc +1)."-06-01' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".((int)$namhoc+1)."-01-01')) then 1 else 0 END) 'HKII2'"),
			DB::raw("MAX(CASE when level_new <> '' and qlhs_profile.profile_year < '".($namhoc +1)."-12-31' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".((int)$namhoc+1)."-05-31')) then 1 else 0 END) 'HKI3'"))
		->where('profile_year','<',$namhoc.'-06-01')
		->where('profile_school_id','=',$truong)
		->whereIn('profile_subject_subject_id',[19,26,27,28,23])
		->groupBy('profile_id','profile_name','profile_year','level_old','level_cur','level_new','kp1.money');

		$data11 = DB::table(DB::raw("({$data1->toSql()}) as m"))->mergeBindings( $data1 )->select('HKII1','HKI2','HKII2','HKI3','m.money','m.DT1','m.DT2','m.DT3','m.DT4','m.profile_id','m.profile_subject_subject_id','m.profile_name','m.profile_year','m.level_old','m.level_cur','m.level_new',DB::raw('CASE 
	when m.profile_subject_subject_id in (19,26,27,28,23) then ((m.HKII1*5*m.money)+m.HKI2*4*(m.money))
END NhuCau'),DB::raw('CASE 
	when m.profile_subject_subject_id in (19,26,27,28,23) then ((m.HKII2*5*m.money)+m.HKI3*4*(m.money))
END DuToan'));
		...
		$data22 = DB::table(DB::raw("({$data2->toSql()}) as m"))->mergeBindings( $data2 )->select('HKII1','HKI2','HKII2','HKI3','m.money','m.DT1','m.DT2','m.DT3','m.DT4','m.profile_id','m.profile_subject_subject_id','m.profile_name','m.profile_year','m.level_old','m.level_cur','m.level_new',DB::raw('CASE 
	when m.profile_subject_subject_id in (19,26,27,28,23) then ((m.HKII1*5*m.money)+m.HKI2*4*(m.money))
END NhuCau'),DB::raw('CASE 
	when m.profile_subject_subject_id in (19,26,27,28,23) then ((m.HKII2*5*m.money)+m.HKI3*4*(m.money))
END DuToan'));
		...
		->select('kp1.money','profile_id','profile_name','profile_year', 'level_old','level_cur','level_new',
			...
			DB::raw('MAX(CASE when profile_subject_subject_id = 19 then 1 else 0 END) DT4'),
			DB::raw("MAX(CASE when level_old <> '' and qlhs_profile.profile_year < '".$namhoc."-06-01' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".$namhoc."-01-01')) then 1 else 0 END) 'HKII1'"),
			DB::raw("MAX(CASE when level_old <> '' and qlhs_profile.profile_year < '".$namhoc."-12-31' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".$namhoc."-05-31')) then 1 else 0 END) 'HKI2'"),
			DB::raw("MAX(CASE when level_old <> '' and qlhs_profile.profile_year < '".($namhoc + 1)."-06-01' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".((int)$namhoc+1)."-01-01')) then 1 else 0 END) 'HKII2'"),
			DB::raw("MAX(CASE when level_cur <> '' and qlhs_profile.profile_year < '".($namhoc +1)."-12-31' and profile_subject_subject_id in (19,26,27,28,23) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".((int)$namhoc+1)."-05-31')) then 1 else 0 END) 'HKI3'"))
		...
		$data33 = DB::table(DB::raw("({$data3->toSql()}) as m"))->mergeBindings( $data3 )->select('HKII1','HKI2','HKII2','HKI3','m.money','m.DT1','m.DT2','m.DT3','m.DT4','m.profile_id','m.profile_subject_subject_id','m.profile_name','m.profile_year','m.level_old','m.level_cur','m.level_new',DB::raw('CASE 
	when m.profile_subject_subject_id in (19,26,27,28,23) then ((m.HKII1*5*m.money)+m.HKI2*4*(m.money))
END NhuCau'),DB::raw('CASE 
	when m.profile_subject_subject_id in (19,26,27,28,23) then ((m.HKII2*5*m.money)+m.HKI3*4*(m.money))
END DuToan'));
	...
	public function insertReport($data11,$data22,$data33,$truong,$namhoc,$user,$now,$report_name,$report_user_sign,$user_name,$status,$filename_attach,$files,$note){
		...
		if((int)$json['1']> 0 && (int)$json['2']>0 && (int)$json['3']>0){
			//$check = DB::table('qlhs_hosobaocao')->where('report_type','=','HTAT-'.$user.'-'.$truong.''.$namhoc.''.((int)$namhoc+1))->where('report_status','=',0)->count();
			if(trim($files) != ""){
			...
			//}else{
				// DB::table('qlhs_hosobaocao')->where('report_type','=','HTAT-'.$user.'-'.$truong.''.$namhoc.''.((int)$namhoc+1))->where('report_status','=',0)->delete();
    ...
    public function loadMucTheoNamHoc(Request $req){
    	...
    	$start = $req->input('start');
    	$limit = $req->input('limit');
    	//$datas = DB::table('qlhs_hosobaocao')->where('report','=','HTAT');
    ...
    public function download_attach($id){
    	$data = DB::table('qlhs_hosobaocao')->where('report','=','HTAT')->where('report_id','=',$id)->select('report_attach_name')->first(); 
    	$dir = storage_path().'/files/HTAT/'.$data->report_attach_name;
    	return response()->download($dir,$data->report_attach_name);
    }
    public function delete_report($id){
    	$dir = storage_path().'/files/HTAT';
    	$json = [];
    	$data = DB::table('qlhs_hosobaocao')->where('report','=','HTAT')->where('report_id','=',$id)->select('report_type','report_attach_name')->first(); 
    	$del1 = DB::table('qlhs_hotrotienan')->where('type_code','=',$data->report_type)->delete();
    	if($del1>0){
    		$del2 = DB::table('qlhs_hosobaocao')->where('report','=','HTAT')->where('report_id','=',$id)->delete();
    		if(file_exists($dir.'/'. $data->report_attach_name)){
					//$files->move($dir, $filename_attach.'-'.$time);	
			 		File::delete($dir.'/'. $data->report_attach_name);	
			}

			if (file_exists(storage_path().'/exceldownload/HTAT/'.$data->report_type.'.xlsx')) {
				File::delete(storage_path().'/exceldownload/HTAT/'.$data->report_type.'.xlsx');
			}
    ...
    public function sendHTAT($id){
    	try{
	    	...
	    	$HTAT = DB::table('qlhs_thamdinh')->insert([
	    			'thamdinh_name' => 'HTAT-'.$string,
					'thamdinh_type' => 'HTAT',
					'thamdinh_hoso_id' => $id,
    ...
    public function downloadfile_Export($id){
    	$data = DB::table('qlhs_hosobaocao')->where('report','=','HTAT')->where('report_id','=',$id)->select('report_type')->first(); 
    	$dir = storage_path().'/exceldownload/HTAT/'.$data->report_type.'.xlsx';
